...
export async function POST(req: NextRequest) {
  ...
  if (!session) {
    return NextResponse.json(
      { error: "Unauthorized", message: "คุณไม่ได้ยืนยันตัวตน", code: "UNAUTHORIZED" },
      { status: 401 }
    );
  }
  ...
  if (session.user.role !== "teacher" || !session.user.isAdmin) {
    return NextResponse.json(
      { error: "Forbidden", message: "เฉพาะแอดมินเท่านั้นที่สามารถรีเซ็ตข้อมูลได้", code: "FORBIDDEN" },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    switch (resetType) {
      ...
      case "specific_student":
        if (!studentId) {
          await conn.rollback();
          return NextResponse.json(
            { error: "Bad Request", message: "กรุณาระบุรหัสนักเรียน", code: "STUDENT_ID_REQUIRED" },
            { status: 400 }
          );
        }
        ...
        await conn.execute(`DELETE FROM ${attendanceTable} WHERE STUDENT_ID = ?`, [studentId]);
        
        if (affectedRows === 0) {
          await conn.rollback();
          return NextResponse.json(
            { error: "Not Found", message: "ไม่พบนักเรียนรหัสนี้", code: "STUDENT_NOT_FOUND" },
            { status: 404 }
          );
        }
      ...
      default:
        await conn.rollback();
        return NextResponse.json(
          { error: "Bad Request", message: "ประเภทการรีเซ็ตไม่ถูกต้อง", code: "INVALID_RESET_TYPE" },
          { status: 400 }
        );
    }
    ...
    console.log(`[Admin Reset] ${session.user.name} (${session.user.id}) performed ${resetType} reset. Affected: ${affectedRows}`);
    ...
    return NextResponse.json(
      { error: "Internal Server Error", message: error, code: "INTERNAL_ERROR" },
      { status: 500 }
    );
  } finally {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [totalStudents] = await conn.query(`SELECT COUNT(*) as count FROM ${studentTable}`);
    
    // นับจำนวนการเช็คชื่อทั้งหมด
    const [totalAttendance] = await conn.query(`SELECT COUNT(*) as count FROM ${attendanceTable}`);
