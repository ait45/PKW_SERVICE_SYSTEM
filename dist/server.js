...
import { autoCutoffWithRetry } from "./src/scripts/checkCutoff.js";
...
app.prepare().then(() => {
    ...
    server.use(async (req, res, next) => {
        ...
        try {
            ...
            if (!pathname.startsWith("/api/auth")) {
                ...
                if (!systemStatus.main_active &&
                    !isAllowed &&
                    !isInternalAsset &&
                    pathname !== "/") {
                    // ✅ ใช้ AND (&&) ไม่ใช่ OR (||)
                    const isTeacherOrAdmin = token?.role === "teacher" && token?.isAdmin === true;
                    const notAuthOrSystemOff = !pathname.startsWith("/api/auth") &&
    ...
    server.listen(port, (err) => {
        ...
        (async () => {
            try {
                ...
                if (result.skipped) {
                    ...
                    console.log(`> Initial autoCutoff completed: ${result.studentsMarked} students marked`);
                }
            }
            catch (err) {
        ...
        setInterval(async () => {
            try {
                ...
                if (!result.skipped && result.success) {
                    console.log(`[autoCutoff] Completed: ${result.studentsMarked} students marked absent`);
                }
            }
            catch (err) {
        ...
        setInterval(() => {
            ...
            console.log("เวลา: ", now.toLocaleTimeString("th-TH", {
                hour: "2-digit",
                minute: "2-digit",
            }));
        }, 60000); // ทุก 1 นาที
