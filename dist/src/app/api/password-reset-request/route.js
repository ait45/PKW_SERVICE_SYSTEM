...
export async function GET(req) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, data: { requests, stats: statsMap }, code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
...
export async function POST(req) {
    try {
        ...
        if (!studentId) {
            return NextResponse.json({ error: "Bad Request", message: "กรุณากรอกรหัสนักเรียน" }, { status: 400 });
        }
        await MongoDBConnection();
...
export async function DELETE(req) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, message: "ลบสำเร็จ", code: "DELETE_DATA_SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
