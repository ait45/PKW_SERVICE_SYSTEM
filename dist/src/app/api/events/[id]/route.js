...
export async function PUT(req, { params }) {
    ...
    try {
        ...
        const { name, type, description, eventDate, startTime, endTime, targetClasses, periods, } = body;
        conn = await MariaDBConnection.getConnection();
        const query = `
    ...
    `;
        ...
        return NextResponse.json({ success: true, message: "แก้ไขกิจกรรมสำเร็จ", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
...
export async function DELETE(req, { params }) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, message: "ลบกิจกรรมสำเร็จ", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        return NextResponse.json({
            error: "Internal Server Error",
