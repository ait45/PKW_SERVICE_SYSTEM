...
async function getStudentClass(conn, studentId) {
    try {
        const result = await conn.query(`SELECT CLASSES FROM ${TABLE_STUDENTS} WHERE STUDENT_ID = ?`, [studentId]);
    ...
    try {
        ...
        if (markAll) {
            ...
            try {
                ...
                return NextResponse.json({ success: true, message: "Marked all as read", code: "SUCCESS" }, { status: 200 });
            }
            finally {
                if (conn)
                    conn.release();
        ...
        await NotificationRead.updateOne({ notificationId, userId }, { $set: { userRole, readAt: new Date() } }, { upsert: true });
        return NextResponse.json({ success: true, message: "Marked as read", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
