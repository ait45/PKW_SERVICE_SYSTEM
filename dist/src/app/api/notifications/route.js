...
export async function POST(req) {
    ...
    `;
        ...
        return NextResponse.json({ success: true, message: "สร้างประกาศสำเร็จ", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
...
export async function PUT(req) {
    ...
    if (session.user.role !== "teacher") {
        return NextResponse.json({ error: "Forbidden", message: "ไม่มีสิทธิ์", code: "FORBIDDEN" }, { status: 403 });
    }
    let conn;
    try {
        const body = await req.json();
        const { id, title, content, type, targetAudience, expireDate, isPinned, status, } = body;
        if (!id) {
            return NextResponse.json({
                error: "Bad Request",
    ...
    `;
        ...
        return NextResponse.json({ success: true, message: "แก้ไขประกาศสำเร็จ", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
...
export async function DELETE(req) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, message: "ลบประกาศสำเร็จ", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
