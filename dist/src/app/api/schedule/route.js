...
export async function PUT(req) {
    ...
    try {
        ...
        if (!id) {
            return NextResponse.json({ error: "Bad Request", message: "ต้องระบุ id", code: "BAD_REQUEST" }, { status: 400 });
        }
        conn = await MariaDBConnection.getConnection();
        const query = `
...
export async function DELETE(req) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, message: "ลบรายการสำเร็จ", code: "DELETE_SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
