...
export async function DELETE(req) {
    ...
    if (!id) {
        return NextResponse.json({ error: "Bad Request", message: "id จำเป็นต้องกรอก", code: "SUCCESS" }, { status: 400 });
    }
    try {
        await MongoDBConnection();
        await LineLog.findByIdAndDelete(id);
        return NextResponse.json({ success: true, message: "ลบ log สำเร็จ", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
