...
export async function POST(req) {
    ...
    try {
        if (await attendanceStart()) {
            ...
            const query = `INSERT INTO ${attendance_Table} (HANDLER, STUDENT_ID, NAME, CLASSES, STATUS) SELECT ?, STUDENT_ID, NAME, CLASSES, ? FROM ${students_Table} WHERE STUDENT_ID = ?`;
...
export async function GET(req) {
    ...
    try {
        ...
        if (data.length === 0)
            return NextResponse.json({ error: "Not Found", message: "ไม่พบข้อมูล", code: "NOT_FOUND" }, { status: 404 });
...
export async function PUT(req) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, message: "แก้ไขข้อมูลเสร็จสิ้น", code: "SUCCESS" }, { status: 200 });
    }
    catch (error) {
        console.error(error);
        return NextResponse.json({
