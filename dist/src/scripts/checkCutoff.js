...
export async function cutoff() {
    ...
    const [hourLate, minuteLate] = settings.lateThreshold
        .split(":")
        .map(Number);
...
export async function attendanceStart() {
    ...
    const [h, m] = setting.absentThreshold
        .split(":")
        .map(Number);
...
export async function autoCutoff() {
    ...
    try {
        ...
        const missing = students.filter((student) => !checkedIds.has(student.STUDENT_ID));
        if (missing.length === 0) {
        ...
        try {
            ...
            const insertData = missing.map((student, index) => {
 ...
 */
export async function autoCutoffWithRetry(maxRetries, retryDelayMs) {
    ...
    const retries = maxRetries ?? config.autoCutoff?.maxRetries ?? DEFAULT_MAX_RETRIES;
    const baseDelay = retryDelayMs ?? config.autoCutoff?.retryDelayMs ?? DEFAULT_RETRY_DELAY_MS;
    let lastError;
 ...
 */
export async function getAutoCutoffStatus() {
    const now = new Date();
    const setting = await readConfig();
    const [h, m] = setting.absentThreshold
        .split(":")
